...
# read in the data
mdata <- read.table("h://mbg_db_reporting//reports//20150316_cans_counts_by_sub_month_nonnineMCI_nonstddataset.csv",
  header=TRUE, sep=",")
...
mdata2$sameMCInonstd <-mdata2$nonnineMCI==mdata2$nonminstd
...
mydata$ylabel_vector[mydata$variable=="nonnineMCI"] <- (-.5)*mydata$maxofrows
...
## change value to "=" if nonnineMCI is same as nonminstd
mydata$value[mdata2$sameMCInonstd & mydata$variable =="nonnineMCI"]<-"="
...
mydata$pointcolor[mydata$variable =="nonnineMCI"] <-"mediumpurple3"
...
xyplot(mydata$value ~ mydata$yrmo |factor(mydata$COUNTY_ID),
 ...
 ##saves file as pdf
pdf(file="h://mbg_db_reporting//reports//20150325_CANSbymo_with_issue_records.pdf", paper="letter", height = 10) 
   ...
   prepanel = function(x, y, subscripts) { 
         list(ylim=extendrange(mydata$ylabel_vector[subscripts], f=.25)) 
			 ...
			 key=list( title="Number of", x = .7, y =1, corner = c(0, 0),
			 				text=list(labels=c("CANS", "Non-9-Digit MCI", "Non-Min. Std. Dataset")),
...
scales=list(
 ...
 type="l",  layout = c(1,6), xlab="Month", 
	ylab="Number of CANS Forms Submitted That Month",
 pch=NA_integer_,
 lty=c(1, 0, 0), 
 main="Number of CANS Submissions \nby County and Month",
