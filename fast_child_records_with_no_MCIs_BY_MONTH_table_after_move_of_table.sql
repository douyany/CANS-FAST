...
SELECT CountyNum, SubmitMo, count(ConcattedMCI) as CountofRecords 
from
(
...
SELECT tbl_child.[COUNTY_ID] as CountyNum, 
		...
		month(cast(tbl_family.[DATE] as datetime)) as SubmitMo   
...
/* limits the time window to (July 2013 to end of October 2014) */
 ...
 /* restriction for latest months */
  and
 month(cast(tbl_family.[DATE] as datetime))>7
 and 
 /* from the most recent year */
 year(cast(tbl_family.[DATE] as datetime))=2014
 ) as TableLayer1
 where
 ...
 group by CountyNum, SubmitMo
 /* sort by the county code */
 order by CountyNum, SubmitMo

 
 /* to get the total for all counties, */
 /* remove the County Number from the grouping */
 /* also remove the County Number as one of the fields in the output table */
 group by SubmitMo
 order by SubmitMo